...
// Asset-type enum GåÆ human-facing label (existing logic)
...
// G£à NEW: Posting categories to show (order matters; after Asset Creation)
...
const TEAM_ID_BY_CATEGORY: Record<string, string> = {
  ...
  // G£à NEW: both go to social-team
...
// G£à Dropdown options (Asset Creation, then new posting categories)
...
function makeDisplayLabel(
  ...
  return `${safeName(a)} GÇö ${active} active (P:${s.P} | IP:${s.IP} | O:${
    s.O
  } | R:${s.R}) GÇó W:${weight}`;
}
...
function enrichAgentsBasic(base: Agent[]): AgentWithLoad[] {
  ...
  const enriched: AgentWithLoad[] = base.map((a) => ({
    ...
    activeCount: 0,
    weightedScore: 0,
    displayLabel: `${safeName(a)} GÇö Available`,
  }));
...
// G£à NEW: decide which UI category a task belongs to
...
const enrichedAgentsFetcher = async (teamId?: string) => {
  ...
  // GÜí OPTIMIZED: Use basic enrichment (no individual fetches)
...
export default function TaskDistributionForClient() {
  ...
  // GÜí OPTIMIZED: Removed useState, using useMemo instead for better performance

  // G£à Agent lists + source toggle via SWR
  ...
  // G£à NEW: Reassign modal state at page level
  const [isReassignModalOpen, setIsReassignModalOpen] = useState(false);

  // GÜí OPTIMIZED: Use useMemo instead of useEffect + setState to prevent unnecessary re-renders
  ...
  // G£à NEW: Get selected tasks as Task objects for modal
  ...
  // G£à NEW: Handle reassign functionality at page level
  ...
  const submitTaskDistribution = async () => {
    if (categoryAssignments.length === 0) {
      toast.warning("GÜán+Å No Assignments", {
        description:
    ...
    try {
      ...
      toast.error("G¥î Task Distribution Failed", {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-gray-100 to-zinc-100">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {client.package?.name && (
                          ...
                          >
                            <span className="mr-1.5">=ƒôª</span>
                ...
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200/70 rounded-2xl p-2 md:p-4">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    >
                      ...
                      {stats.fullyAssigned ? " GÇó All Set G£ô" : ""}
                    </Badge>
                  </div>
              ...
              <section aria-labelledby="tasks-heading" className="space-y-8">
                ...
                {loading ? (
                  ...
                  <TaskTabs
                    ...
                    // G£à NEW: Modal props from parent
                  ...
                  <TaskTabs
                    ...
                    onNoteChange={handleNoteChange}
                    onViewModeChange={setViewMode}
                    // G£à NEW: Pass modal handlers from parent
                    isReassignModalOpen={isReassignModalOpen}
                    onReassignModalOpen={setIsReassignModalOpen}
                    onReassign={handleReassign}
                    onUnassign={handleUnassign}
      ...
      {/* G£à NEW: Reassign Modal at page level */}
      <ReassignModal
